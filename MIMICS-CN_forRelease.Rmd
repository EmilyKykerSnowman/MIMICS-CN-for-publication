...
LTER_SITE_1.csv
LIDET_SITE_obs/Archive folder
DAYCENT_LIDET_all_trimmed.csv
histogramDataRedone.csv
...
plotSteadyState <- function(){
    ...
    for (d in 1:nday)  {
      for (h in 1:24)   {
        ...
        if (doy == 365) {
          doy <- 1
          print(paste(c("finished spinup year", yearnum[d])))
        } else {
          doy <- doy + 1
...
dfC=data.frame(yearnum,
               SOM[1,], SOM[2,], SOM[3,],
               MIC[1,], MIC[2,],
               LIT[1,],LIT[2,])
dfC=subset(dfC, yearnum>spinupYears-5)
...
dfN=data.frame(yearnum,
               SOMN[1,], SOMN[2,], SOMN[3,],
               MICN[1,], MICN[2,],
               LITN[1,],LITN[2,],
               InorgN[1,])
dfN=subset(dfN,yearnum>spinupYears-5)
...
CStacked = ggplot(dataLongC, aes(x=dataLongC$yearnum,y=dataLongC$MassC,fill=dataLongC$Pool))+geom_area(position='stack')
...
NStacked = ggplot(dataLongN, aes(x=dataLongN$yearnum,y=dataLongN$MassN,fill=dataLongN$Pool))+geom_area(position='stack')
...
NsourceStacked = ggplot(dataLongNsource, aes(x=dataLongNsource$yearnum,y=dataLongNsource$MassN,fill=dataLongNsource$Pool))+geom_area(position='stack')
...
plot(yearnum,  LIT[1,], lwd=3, ylim=c(min(LIT)*0.7, max(LIT))*1.15, type ="l", xlab="", main = paste(strSite[s]))
lines(yearnum, LIT[2,], lwd=3, col = 2)
legend("topright", legend=c("Met","Struc"), col=c(1,2), lty = 1,
       lwd = 3, cex=1.3, bty="n")

plot(yearnum,  MIC[1,], lwd=3, type ="l", xlab="", ylim=c(min(MIC)*0.7, max(MIC))*1.15)
lines(yearnum, MIC[2,], lwd=3, col = 2)
legend("topright", legend=c("Mic_r","Mic_K"), col=c(1,2), lty = 1,
       lwd = 3, cex=1.3, bty="n")

plot(yearnum,  SOM[1,], lwd=3, type ="l", ylim=c(min(SOM)*0.7, max(SOM))*1.15)
lines(yearnum, SOM[2,], lwd=3, col = 2)
lines(yearnum, SOM[3,], lwd=3, col = 4)
legend("topright", legend=c("Phys","Chem","Avail"), col=c(1,2,4), lty = 1,
       lwd = 3, cex=1.3, bty="n")

par(mfrow=c(4,1), mar=c(1,1,1,1))
plot(yearnum,  LITN[1,], lwd=3, ylim=c(min(LITN)*0.7, max(LITN))*1.15, type ="l", xlab="", main = paste(strSite[s]," N"))
lines(yearnum, LITN[2,], lwd=3, col = 2)
legend("topright", legend=c("Met","Struc"), col=c(1,2), lty = 1,
       lwd = 3, cex=1.3, bty="n")

plot(yearnum,  MICN[1,], lwd=3, type ="l", xlab="", ylim=c(min(MICN)*0.7, max(MICN))*1.15)
lines(yearnum, MICN[2,], lwd=3, col = 2)
legend("topright", legend=c("Mic_r","Mic_K"), col=c(1,2), lty = 1,
       lwd = 3, cex=1.3, bty="n")

plot(yearnum,  SOMN[1,], lwd=3, type ="l", ylim=c(0, max(SOMN))*1.15)
lines(yearnum, SOMN[2,], lwd=3, col = 2)
lines(yearnum, SOMN[3,], lwd=3, col = 4)
lines(yearnum, InorgN[1,], lwd=3, col = 6)
legend("topright", legend=c("Phys","Chem","Avail","Inorg N"), col=c(1,2,4,6), lty = 1,
       lwd = 3, cex=1.3, bty="n")

plot(yearnum,  InorgN[1,], lwd=3, type ="l", ylim=c(0, max(InorgN))*1.15)
lines(yearnum, LLoss[1,], lwd=3, col = 4)
...
p1 = ggplot(histogramData,aes(x=`SOC`, fill=Study)) + geom_density(alpha=0.25) +
          scale_x_log10(breaks = trans_breaks("log10", function(x) 10^x),
          labels = number_format(accuracy = 0.01))+#trans_format("log10", math_format(10^.x)))  + 
          labs(x = "Soil organic carbon (% by mass)") +
...
p3 = ggplot(histogramData,aes(x=`MBC`, fill=Study)) + geom_density(alpha=0.25)+
          scale_x_log10(breaks = trans_breaks("log10", function(x) 10^x),
          labels = number_format(accuracy = 0.0001))+#trans_format("log10", math_format(10^.x))) + 
          labs(x = "Microbial biomass carbon (% by mass)")+
...
p5 = ggplot(histogramData,aes(x=`MBC.as...of.soil.C`, fill=Study)) + geom_density(alpha=0.25)+
          scale_x_log10(breaks = trans_breaks("log10", function(x) 10^x),
          labels = number_format(accuracy = 0.1))+#trans_format("log10", math_format(10^.x))) + 
          labs(x = "Microbial biomass C as a % of soil C")+
...
p7 = ggplot(histogramData,aes(x=`Soil.C.N`, fill=Study)) + geom_density(alpha=0.25)+
          scale_x_log10(breaks = trans_breaks("log10", function(x) 10^x),
          labels = number_format(accuracy = 1))+#trans_format("log10", math_format(10^.x))) + 
          labs(x = "Soil C:N")+
    ...
    for( i in 1:6)     {    #SPEEDS search
      ...
      for (d in 1:nday)  {
        ...
        if (stackedExpFigs==TRUE){
          ...
          TCStacked = ggplot(dataLongTC, aes(x=dataLongTC$yearnum,y=dataLongTC$MassC,fill=dataLongTC$Pool))+geom_area(position='stack')
          ...
          TNStacked = ggplot(dataLongTN, aes(x=dataLongTN$yearnum,y=dataLongTN$MassN,fill=dataLongTN$Pool))+geom_area(position='stack')
...
outputLitterbags<-function(){
  ...
  #---------------Calculate averages across litter types--------------------------
    ...
    TLITleft <<- array(NA, dim=c(6,nday), dimnames=list(LITtype,c(as.character(yearnum))))
    allTCleft <<- array(NA, dim=c(6,nday), dimnames=list(LITtype,c(as.character(yearnum))))
    ...
    TLITNleft <<- array(NA, dim=c(6,nday), dimnames=list(LITtype,c(as.character(yearnum))))
    allTNleft <<- array(NA, dim=c(6,nday), dimnames=list(LITtype,c(as.character(yearnum))))
    ...
    for (y in 1:nobsyears) {
      obsindex[y] <- which.min(abs(yearnum-obsyear[y]))
    }
...
### Function to plot 6-panel figure with various SS fractions relationships
```{r}
fractions6panelPlot<-function(){

attach(LTERdata)  
MATtall = rep(MAT,times=3)
LIGtall = rep(LIG,times=3)
attach(SSoutput)
SOMPool = c(rep("SOMp",times=14),rep("SOMc",times=14),rep("SOMa",times=14))
SOMamount = c(SOMp,SOMc,SOMa)
FractionsBarPlot = data.frame(MATtall,LIGtall,SOMPool,SOMamount)

ANPP.CLAY2 = ANPP*CLAY2	
SoilCN = (MICr+MICK+LITm+LITs+SOMp+SOMc+SOMa)/(MICrN+MICKN+LITmN+LITsN+SOMpN+SOMcN+SOMaN+InorgN)
FractionsFigure = data.frame(SSoutput,LTERdata,ANPP.CLAY2,SoilCN)

unevenBars = ggplot(FractionsBarPlot, aes(fill=SOMPool, y=SOMamount, x=MATtall)) + 
  geom_bar(position="stack", stat="identity",width=0.8) + 
  scale_fill_manual(values=c("#F8766D","#00BA38","#619CFF"))+
  theme(legend.position = c(0.25, 0.8)) + 
  labs(x = "Mean annual temperature (degrees C)") +
  labs(y = expression('Total soil carbon (mg C cm'^'-3'*')'))+
  #geom_text(aes(x=-10,y=50,label="(a)"))
  annotate("text",x=-10,y=50,label="(a)")
  
stackedBars = ggplot(FractionsBarPlot, aes(fill=SOMPool, y=SOMamount, x=MATtall)) + 
  geom_bar(position="fill", stat="identity",width=0.8,show.legend=FALSE)+ 
  labs(x = "Mean annual temperature (degrees C)") +
  labs(y = "Fraction of total soil C")+
  #geom_text(aes(x=-10,y=1,label="(b)"))
  annotate("text",x=-10,y=1,label="(b)")

r2A=signif(summary(lm(ANPP.CLAY2~SOMp))[[8]], digits=2)
SOMpCor = ggplot(FractionsFigure, aes(x=ANPP.CLAY2, y=SOMp))+
  geom_point(col="#619CFF") + 
  geom_smooth(method=lm, se=FALSE, fullrange=TRUE,col="#619CFF")+ 
  labs(x = "Product of ANPP and clay content") +
  labs(y = expression('Physicochemically-protected soil C (mg C cm'^'-3'*')'))+
  geom_text(aes(label = paste("R^2: ",r2A,sep="")),parse=T,x=20000,y=20, show_guide=F,col="#619CFF")+
  #geom_text(aes(x=0,y=50,label="(c)"))
  annotate("text",x=0,y=50,label="(c)")

r2B=signif(summary(lm(MAT~SOMa))[[8]], digits=2)
r2C=signif(summary(lm(MAT~SOMc))[[8]], digits=2)
SOMMATCor = ggplot(subset(FractionsBarPlot,SOMPool!="SOMp"), 
  aes(x=MATtall, y=SOMamount, col=SOMPool))+
  scale_colour_manual(values=c("#F8766D","#00BA38","#619CFF"))+
  geom_point(show.legend=FALSE) + 
  geom_smooth(method=lm, se=FALSE, fullrange=TRUE) + 
  theme(legend.position = c(0.25, 0.8)) + 
  labs(x = "Mean annual temperature (degrees C)") +
  labs(y = expression('Soil C (mg C cm'^'-3'*')'))+
  #geom_text(aes(x=-10,y=15,label="(d)"))+
  annotate("text",x=-10,y=15,label="(d)")+
  geom_text(aes(label = paste("R^2: ",r2B,sep="")),parse=T,x=-2,y=2, show_guide=F,col="#F8766D")+
  geom_text(aes(label = paste("R^2: ",r2C,sep="")),parse=T,x=15,y=5, show_guide=F,col="#00BA38")

r2D=signif(summary(lm(LIG~SOMa))[[8]], digits=2)
r2E=signif(summary(lm(LIG~SOMc))[[8]], digits=2)
SOMLIGCor = ggplot(subset(FractionsBarPlot,SOMPool!="SOMp"), 
  aes(x=LIGtall, y=SOMamount,col=SOMPool,show.legend=FALSE))+
  scale_colour_manual(values=c("#F8766D","#00BA38","#619CFF"))+
  geom_point(show.legend=FALSE) + 
  geom_smooth(method=lm, se=FALSE, fullrange=TRUE,show.legend=FALSE) + 
  labs(x = "Litter lignin content (%)") +
  labs(y = expression('Soil C (mg C cm'^'-3'*')'))+
  #geom_text(aes(x=15,y=15,label="(e)"))+
  annotate("text",x=15,y=15,label="(e)")+
  geom_text(aes(label = paste("R^2: ",r2D,sep="")),parse=T,x=23,y=1.5, show_guide=F,col="#F8766D")+
  geom_text(aes(label = paste("R^2: ",r2E,sep="")),parse=T,x=18,y=6, show_guide=F,col="#00BA38")

r2F=signif(summary(lm(SOMp~SoilCN))[[8]], digits=2)
SOMpvCN = ggplot(FractionsFigure, aes(x=SOMp, y=SoilCN))+
  geom_point(col="#619CFF") + 
  geom_smooth(method=lm, se=FALSE, fullrange=TRUE,col="#619CFF")+ 
  labs(x = expression('Physicochemically-protected soil C (mg C cm'^'-3'*')')) +
  labs(y = "Soil C:N") +
  geom_text(aes(label = paste("R^2: ",r2F,sep="")),parse=T,x=18,y=10.5, show_guide=F,col="#619CFF")+
  #geom_text(aes(x=0.1,y=13,label="(f)"))
  annotate("text",x=0.1,y=13,label="(f)")

fout          <- c('Display_Items/Fractions6PanelFigure.pdf')
fout          <- paste(fout, collapse="")
pdf(fout, width=12, height=8)
par(mar=c(5,5,1,1), mfrow=c(2,2))

grid.arrange(unevenBars, stackedBars, SOMpCor, SOMMATCor, SOMLIGCor, SOMpvCN, ncol = 3, 
